...
public class lawyermotderb_req extends ActionBarActivity implements SearchView.OnQueryTextListener {
    RecyclerView recycleView;
    lawyermotderb_req_adapter adapter;
    List<lawyer> lawyermotderb_Req=new ArrayList<>();
    static int ID;
    final String lm_url="http://mashowra.com/oapi/lawyers6.php";
    boolean x=false;

    @Override
    protected void onCreate(Bundle savedInstanceState) {
        super.onCreate(savedInstanceState);
        setContentView(R.layout.activity_lawyermotderb_req);
        recycleView = (RecyclerView) findViewById(R.id.RecyclerView_lawyermotderb_req);
        ...
        Runnable run = new Runnable() {
            ...
            public void run() {
                try {
                    java.net.URL lawyermotderburl = new URL(lm_url);
                    HttpURLConnection con = (HttpURLConnection) lawyermotderburl.openConnection();
                    ...
                    try {
                        ...
                        JSONArray lawyermotderbarray=new JSONArray(result);

                        for(int i=0;i<lawyermotderbarray.length();i++)
                        {

                            JSONObject json_data = lawyermotderbarray.getJSONObject(i);
                            lawyer l = new lawyer();
                    ...
                    runOnUiThread(
                            new Runnable() {
                                ...
                                public void run() {
                                    ...
                                    adapter = new lawyermotderb_req_adapter(lawyermotderb_req.this,lawyermotderb_Req);
        ...
        recycleView.addOnItemTouchListener(
                new RecyclerItemClickListener2(this, new RecyclerItemClickListener2.OnItemClickListener() {
                    @Override
                    public void onItemClick(View view, int position) {

                        ID=adapter.lawyermotderb_Req.get(position).id;
                        startActivity(i);
    ...
    public boolean onQueryTextSubmit(final String query) {
        int size = adapter.lawyermotderb_Req.size();
        if (size > 0) {
            for (int i = 0; i < size; i++) {
                adapter.lawyermotderb_Req.remove(0);
            }
        ...
        Runnable run = new Runnable() {
            ...
            public void run() {
                try {
                    java.net.URL lawyerurl = new URL(lm_url);
                    ...
                    runOnUiThread(
                            new Runnable() {
                                ...
                                public void run() {
                                    ...
                                    recycleView.setAdapter(adapter);
                                    if(x==false)
                                    {          new AlertDialog.Builder(lawyermotderb_req.this)
                                            .setTitle("البحث عن محامي متدرب/محامية متدربة")
